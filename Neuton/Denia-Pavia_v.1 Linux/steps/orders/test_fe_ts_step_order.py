...
class TestFeTsStepOrder(BaseStepOrder):
    @staticmethod
    def get_steps() -> List[ProcessingStep]:
        local = not os.path.exists('data_preprocessing')
        final_dump_path = 'output/processed_test.csv' if local else 'data_preprocessing/output/processed_test.csv'
        return [MapMetricStep(),
                ...
                SaveToCsvStep(path='data_preprocessing/output/processed_test.csv', save_index=False),
