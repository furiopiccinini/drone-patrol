import pandas as pd
import numpy as np
import pickle, timeit, datetime, sys
import os
...
def transform_test_from_dict(data, mean_target_encoding = False):
    '''
    - binarize
    - one-hot-encode
    ...
    print('\n   - Transform Test according to Train started at: {}'.format(datetime.datetime.now().time().strftime('%H:%M:%S')))
    ...
    processed_train_header = pickle.load(open('dict/processed_train_header.p','rb'))

    path = 'dict/droped_columns_after_outliers.p'
    ...
    #     encoder = pickle.load(open('dict/cat_encoder.p', 'rb'))
    ...
    # If test set contained additional categories to the binary columns from train binary_features_dict, they will have
